...
def square_distance(pointA, pointB):
    ...
    return sum([(pointA[dimension] - pointB[dimension])**2  for dimension in xrange(dimensions)])
...
class KDTreeNeighbours(object):
    ...
    def add(self, point):
        sd = square_distance(point[:-1], self.query_point)
    ...
    def __init__(self, data):
        def pre_process(data_dict):
            ...
            for label in data_dict:
                ret.append(data_dict[label]["center"] + [label])
            return ret
        ...
        self.root_node = build_kdtree(pre_process(data), depth=0)
